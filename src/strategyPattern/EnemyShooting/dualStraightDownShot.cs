...
{
    public class DualStraightDownShot : IBaseShootingStrategy
    {
        public DualStraightDownShot()
        {
            _Speed = 3.5f;
        ...
        public override void Shoot(GameTime gameTime, GraphicsDeviceManager graphics, Vector2 position, List<Bullet> bullets)
        {
            ...
            if (shootCoolDown <= 0)
            {
                bullets.Add(new BulletEnemy(Textures.textureBulletE, new Vector2(position.X + 5, position.Y), _Speed));
                bullets.Add(new BulletEnemy(Textures.textureBulletE, new Vector2(position.X - 5, position.Y), _Speed));
